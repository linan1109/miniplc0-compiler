            ...
            switch (current_state) {
                ...
                case IDENTIFIER_STATE: {
                    ...
                    if (!current_char.has_value()) {
                        ...
                        if (str == "begin")
                            return std::make_pair(std::make_optional<Token>(TokenType::BEGIN, str, pos, currentPos()),
                                                  std::optional<CompilationError>());
                        else if (str == "end")
                            return std::make_pair(std::make_optional<Token>(TokenType::END, str, pos, currentPos()),
                                                  std::optional<CompilationError>());
                        else if (str == "const")
                            return std::make_pair(std::make_optional<Token>(TokenType::CONST, str, pos, currentPos()),
                                                  std::optional<CompilationError>());
                        else if (str == "var")
                            return std::make_pair(std::make_optional<Token>(TokenType::VAR, str, pos, currentPos()),
                                                  std::optional<CompilationError>());
                        else if (str == "print")
                            return std::make_pair(std::make_optional<Token>(TokenType::PRINT, str, pos, currentPos()),
                                                  std::optional<CompilationError>());
                        return std::make_pair(std::make_optional<Token>(TokenType::IDENTIFIER, str, pos, currentPos()),
                                              std::optional<CompilationError>());
                    }

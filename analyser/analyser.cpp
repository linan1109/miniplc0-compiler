...
namespace miniplc0 {
    ...
    std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t &out) {
        ...
        auto next = nextToken();
        if (!next.has_value())
            return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrInvalidConstantExpression);
        int32_t i = 1;
    ...
    std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
        ...
        if(isUninitializedVariable(str))
            changeToInitializedVariable(str);

        // 需要生成指令吗？
        int32_t index = getIndex(str);
    ...
    void Analyser::changeToInitializedVariable(const std::string &s){
        _vars[s] = _uninitialized_vars[s];
        _uninitialized_vars.erase(s);
    }
